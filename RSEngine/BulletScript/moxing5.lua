...
function initialize()
	...
	maxFrame = 120 -- max frame count, if current frame is bigger than maxFrame, then the bulletstorm will be removed from system
...
function updateFrame()
	...
	local tmpt = 1
		for i=1, 100 do 
			...
			vetx[tmpt] = 400 - currentFrame * 2  + i * 10
			tmpt = tmpt + 1
			vetx[tmpt] = 2*(currentFrame % 800) * math.sin((i % 50)*0.02 * 2 * 3.1415926)	
			tmpt = tmpt + 1
			vetx[tmpt] = (currentFrame % 800) * math.cos((i % 50)*0.02 * 2 * 3.1415926)
			tmpt = tmpt + 1
		end
		for i=1, 100 do 
			--vetx[(i - 1) * 3 + 1] = math.sin(currentFrame - i)
		...
		for i=1, 100 do 
			...
			vetx[tmpt] = 400 - currentFrame * 2 + i * 10
			tmpt = tmpt + 1
			vetx[tmpt] = (currentFrame % 800) * math.sin((i % 50)*0.02 * 2 * 3.1415926)		
			tmpt = tmpt + 1
			vetx[tmpt] = (currentFrame % 800) * math.cos((i % 50)*0.02 * 2 * 3.1415926)
			tmpt = tmpt + 1
		end
...
function updateInterpolation(intp)
	...
	local tmpt = 1
		for i=1, 100 do 
			--vetx[(i - 1) * 3 + 1] = math.sin(currentFrame - i)
			vetx[tmpt] = 400 - currentFrame * 2 + i * 10 - intp
			tmpt = tmpt + 1
			vetx[tmpt] = 2*(currentFrame % 800) * math.sin((i % 50)*0.02 * 2 * 3.1415926)		
			tmpt = tmpt + 1
			vetx[tmpt] = (currentFrame % 800) * math.cos((i % 50)*0.02 * 2 * 3.1415926)
